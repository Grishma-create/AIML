class Graph:
    def __init__(self, graph, heuristic, start):
        self.graph, self.heuristic, self.start = graph, heuristic, start
        self.parent, self.status, self.solution = {}, {}, {}
    ...
    def print_solution(self):
        print("GRAPH SOLUTION:", self.solution)

    def compute_min_cost_children(self, v):
        min_cost, cost_to_children = float('inf'), {}
        cost_to_children[min_cost] = []

        for node_info_list in self.get_neighbors(v):
            cost, node_list = sum(self.get_heuristic(c) + w for c, w in node_info_list), [c for c, _ in node_info_list]

            if min_cost > cost:
                min_cost, cost_to_children[min_cost] = cost, node_list

        return min_cost, cost_to_children[min_cost]

    def ao_star(self, v, back_tracking):
        if self.get_status(v) >= 0:
            min_cost, child_nodes = self.compute_min_cost_children(v)
            self.set_heuristic(v, min_cost)
            self.set_status(v, len(child_nodes))
            solved = all(self.get_status(c) != -1 for c in child_nodes)
